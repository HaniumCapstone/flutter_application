...
class _DashBoardState extends State<DashBoard> {
  List<Person> people = [];
  List<String> personImageFileNames =
      List.generate(19, (index) => 'img${index + 1}.jpeg');
  bool _isLoading = true;

  Future<void> fetchPeopleFromDatabase() async {
    final response = await http
        .get(Uri.parse('http://18.188.95.144/person_data_return.php'));
    if (response.statusCode == 200) {
      ...
      for (var personData in jsonData) {
        final person = Person(
          ...
          imageFileName:
              personImageFileNames[index % personImageFileNames.length],
        );
